...
#     B, C, W, H = ycrcb.shape
#     im_flat = ycrcb.reshape(B, 3, -1).transpose(1, 2)
#     mat = torch.tensor(
#         [[1.0, 1.0, 1.0], [1.403, -0.714, 0.0], [0.0, -0.344, 1.773]]
#     ).to(Y.device)
#     bias = torch.tensor([0.0 / 255, -0.5, -0.5]).to(Y.device)
#     temp = (im_flat + bias)@mat
#     out = temp.transpose(1, 2).reshape(B, C, W, H)
#     out = clamp(out)
#     return out
def YCrCb2RGB_Batch(input_im):
...
def calculateGrad(fusedImg):

    return gradient(fusedImg)
